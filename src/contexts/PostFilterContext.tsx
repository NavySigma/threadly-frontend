...
const DEFAULT: PostFilter = { search: "", tag_id: "", sort: "latest", answer: "all" };
...
export function PostFilterProvider({ children }: { children: ReactNode }) {
  ...
  const hasActiveFilter =
    filter.search !== "" || filter.tag_id !== "" ||
    filter.sort !== "latest" || filter.answer !== "all";

  return (
    <PostFilterContext.Provider value={{ filter, setSearch, setTag, setSort, setAnswer, resetFilter, hasActiveFilter }}>
      {children}
